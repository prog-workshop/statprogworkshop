...
flowchart LR
    ...
    B --> D[Preserves randomization,<br/>less bias]
    C --> E[Assumes all effect<br/>modifiers / prognostic<br/>factors are matched]
```
...
| 2025 | Intervention (Phase 3 study) vs Comparator (Ibru) | PFS, OS, TEAE | `MAIC` (Roche) package; validation runs |
...
- PH-assumption diagnostics: `ph_diagplot()`, `ph_diagplot_lch()`,
...
- After MAIC weighting, **no endpoint** (ORR, CRR, PFS, OS) showed a
  statistically significant difference between the intervention and comparator
...
- **Choose R and package versions deliberately, and reproduce across
...
- Case study: Intervention (studies Pivotal(A + B)) vs. Comparator (ACE-LY-004),
  internal MAIC analysis
